        ...
        try{
            ...
            if(c.loadProblem(p, ProblemType.SBRP, typeDistance)){
                if(heuristicType.equals(HeuristicType.NearestNeighborWithRLC)){
                    ...
                    n.setSizeRLC(rlc, p);
                    p.automatic_assignment(heuristicType, rlc);
                    p.fillCostMatrix(p.getListBusesStop(), p.getListDepots(), typeDistance);
                    c.executeHeuristic(1, heuristicType, p);
                    result = c.getBestSolution();
                }
                else{
                    p.automatic_assignment(heuristicType, rlc);
                    p.fillCostMatrix(p.getListBusesStop(), p.getListDepots(), p.getDistanceUsed());
                    c.executeHeuristic(1, heuristicType, p);
                    result = c.getBestSolution();
                }
